...
router.post('/chat', async (req, res) => {
  ...
  try {
    ...
    // Same routed estimate the form flow uses, priced against the
    // scheduled hour so an AI booking and a form booking for the same
    // trip never quote different numbers.
    const quote = await estimateRoute(pickup, dropoff, d.datetime_iso);
    ...
    const { data: booking, error: dbErr } = await supabase
      ...
      .insert({
        ...
        source: 'ai',
        // Cached here for the same reason as the form flow: every later
        // map render reads these columns instead of re-billing Google.
        route_polyline: quote.polyline ?? null,
        route_source: quote.source === 'google' ? 'google' : 'haversine',
      })
      .select()
      .single();
    ...
    res.json({
      ...
      booking: {
        ...booking,
        trackUrl: base ? `${base}/track/${booking.track_token}` : `/track/${booking.track_token}`,
      },
    });
  } catch (err) {

...
export function useDriverAuth() {
  ...
  // (e.g. a truly incomplete account) can't loop us calling ensure-profile
  // on every auth event.
  ...
  useEffect(() => {
    ...
    async function loadDriver(session) {
      ...
      // Authenticated, no error, but no profile row — the signup trigger
      // never created one. Try once to build it from this account's own
      // signup data, then use whatever comes back. This turns the old
      // "no driver profile, contact support" dead-end into a silent repair.

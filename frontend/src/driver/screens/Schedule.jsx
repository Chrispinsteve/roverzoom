...
export default function Schedule({ onBack, onClaimed }) {
  ...
  const claim = async (bookingId) => {
    ...
    try {
      ...
      // Hand the freshly claimed booking straight back up. Without this
      // the driver claims a trip and is left staring at a list, with no
      // route to the navigation screen for the ride they just took on.
      if (onClaimed) { onClaimed(claimed); return; }
      await load();
      setTab('mine');
    } catch (e) {
      setError(e.message);
